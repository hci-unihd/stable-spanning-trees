/*
 * ApproxBetweennessEdges.cpp
 *
 *  Created on: 09.04.2014
 *      Author: cls
 */

#include <networkit/centrality/ApproxBetweennessEdges.hpp>
...
ApproxBetweennessEdges::ApproxBetweennessEdges(const Graph& G, double epsilon, double delta, double universalConstant, bool computeEdgeCentrality) : Centrality(G, true, computeEdgeCentrality), epsilon(epsilon), delta(delta), universalConstant(universalConstant) {

}


void ApproxBetweennessEdges::run() {
    ...
    if (computeEdgeCentrality) {
    	...
    	// The iteration value written on page 29. TODO: might be wrong
        r = ceil((universalConstant / (epsilon * epsilon)) * (floor(log2(vd - 1)) + 1 - log(delta)));
...
count ApproxBetweennessEdges::numberOfSamples() const {
    assureFinished();
