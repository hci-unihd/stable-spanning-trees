...
from External.ICT.calculate_edge_centrality import calculate_centrality_cluster_all2_c
...
def find_best_edge(G, w, edgeId_to_edge, algorithm_type, cluster_centers=None, ε=None, δ=None, node_weights=None):
    ...
    if algorithm_type == "cluster_all2":
        scores = calculate_centrality_cluster_all2_c(G, cluster_centers, node_weights)
...
def calculate_ICT(G, algorithm_type, cluster_centers=None, ε=0.03, δ=0.1, zeros_stay_zeros=False,
                  update_G=10, create_snaps=False, position=None, folder_name=None, good_edges=None, node_weights=None, log=False):
    ...
    G : networkit graph
        The corresponding graph 
    algorithm_type : str
        The type of the centrality algorithm (exact_own, RK, RK_py, cluster, cluster_py, cluster_all, cluster_all2, cluster_all_py)
    ...
    good_edges=[] : list
        optional, initial value for the edges that will be used definitively in the ICT.
    ...
    G_backup = deepcopy(G)
    # Only needed if the good_edges array is not empty.
    for u, v in good_edges:
        G_backup.setWeight(u, v, w)
        
    skipped = len(good_edges)
    ...
    for i in tqdm(range(G_backup.numberOfNodes()-1), desc="Iteration over all nodes"):
        ...
        edge_id, scores = find_best_edge(G_backup, w, edgeId_to_edge, algorithm_type, cluster_centers, ε, δ, node_weights)
        if zeros_stay_zeros:
...
def calculate_sub_ICTs(position, cluster_centers, labels, t=12, mode = "K_Nearest"):
    
    
    if t<3:
        raise RuntimeError("Please choose a larger t")
    ...
    # Put the clusters with #nodes < threshold to the cluster of the nearest node. Should not be needed for well preperated data
    ...
    labels = np.array(new_node_labels)
    ...
    for component in tqdm(components):
        ...
        if k is not None:
            sub_G, _ = create_graph(sub_number_of_nodes, mode, position=position[component], k=k, beta=2, delta=0.5, bridges=4)
        else:
            sub_G, _ = create_graph(sub_number_of_nodes, mode, position=position[component], beta=2, delta=0.5)
            
        sub_G.indexEdges()

        sub_ICT = calculate_ICT(sub_G, algorithm_type="exact_own", zeros_stay_zeros=True, update_G=1.1)
        sub_ICT.indexEdges()
        sub_ICTs.append(sub_ICT)
        
    return sub_ICTs, components