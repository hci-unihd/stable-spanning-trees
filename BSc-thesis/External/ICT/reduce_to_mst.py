...
def MST(ICT, position, centers):
    ...
    bc = nk.centrality.Betweenness(new_ICT, False, True)
    bc.run()  
    score = np.array(bc.edgeScores())
    score /= np.max(score)
    widths = (np.sqrt(score)*12)
    offset = np.max(widths)

    for u, v, w in new_ICT.iterEdgesWeights():
        new_ICT.setWeight(u,v, -widths[new_ICT.edgeId(u, v)]+offset+1)
