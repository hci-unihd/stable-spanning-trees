...
cdef extern from "<networkit/centrality/ApproxBetweennessEdges.hpp>":

	cdef cppclass _ApproxBetweennessEdges "NetworKit::ApproxBetweennessEdges" (_Centrality):
		_ApproxBetweennessEdges(_Graph, double, double, double, bool_t) except +
		count numberOfSamples() except +
		vector[double] edgeScores() except +


cdef class ApproxBetweennessEdges(Centrality):
	""" Approximation of betweenness centrality according to algorithm described in
 	Matteo Riondato and Evgenios M. Kornaropoulos: Fast Approximation of Betweenness Centrality through Sampling

 	ApproxBetweennessEdges(G, epsilon=0.01, delta=0.1, universalConstant=1.0)
	...
	def __cinit__(self, Graph G, epsilon=0.01, delta=0.1, universalConstant=1.0, computeEdgeCentrality=False):
		self._G = G
		self._this = new _ApproxBetweennessEdges(G._this, epsilon, delta, universalConstant, computeEdgeCentrality)
	...
	def numberOfSamples(self):
		return (<_ApproxBetweennessEdges*>(self._this)).numberOfSamples()



cdef extern from "<networkit/centrality/BA.hpp>":

	cdef cppclass _BA "NetworKit::BA" (_Centrality):
		_BA(_Graph, vector[node]) except +
		vector[double] edgeScores() except +



cdef class BA(Centrality):
	""" Calculate a Estimation of the betweenes using the given cluster centers.
	...
	cluster_centers : ndarray
		The ndarray with the node indices of the cluster centers
	"""

	def __cinit__(self, Graph G, vector[node] cluster_centers):
		self._G = G
		self._this = new _BA(G._this, cluster_centers)
...
cdef extern from "<networkit/centrality/ClusterToAll.hpp>":

	cdef cppclass _ClusterToAll "NetworKit::ClusterToAll" (_Centrality):
		_ClusterToAll(_Graph, vector[node]) except +
		vector[double] edgeScores() except +



cdef class ClusterToAll(Centrality):
	""" Calculate a Estimation of the betweenes using the given cluster centers as the surec nodes and all other nodes as the target nodes
	...
	def __cinit__(self, Graph G, vector[node] cluster_centers):
		self._G = G
		self._this = new _ClusterToAll(G._this, cluster_centers)
...
cdef extern from "<networkit/centrality/ClusterToAll2.hpp>":

	cdef cppclass _ClusterToAll2 "NetworKit::ClusterToAll2" (_Centrality):
		_ClusterToAll2(_Graph, vector[node], vector[float]) except +
		vector[double] edgeScores() except +


cdef class ClusterToAll2(Centrality):
	""" Calculate a Estimation of the betweenes using the given cluster centers as the surec nodes and all other nodes as the target nodes
	...
	def __cinit__(self, Graph G, vector[node] cluster_centers, vector[float] node_weights):
		self._G = G
		self._this = new _ClusterToAll2(G._this, cluster_centers, node_weights)
